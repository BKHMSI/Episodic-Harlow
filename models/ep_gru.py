...
from models.ep_gru_cell import EpGRUCell

@dataclass
class EpGRUCell_Builder:
    hidden_size                 : int
...
class EpGRU(nn.Module):
    def __init__(
        ...
        super().__init__()
        self._cell_builder = EpGRUCell_Builder(*args, **kargs)
    
        Dh = self._cell_builder.hidden_size
        def make(isize: int):
            cell = self._cell_builder.make(isize)
            return EpGRU_Layer(cell, isize, batch_first=batch_first)
